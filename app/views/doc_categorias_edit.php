<?php
// /costos/app/views/doc_categorias_edit.php
$base    = rtrim($this->cfg['BASE_URL'] ?? '/', '/');
$row     = $row ?? null;
$modulos = $modulos ?? [];
if (!$row) { echo '<div class="alert alert-danger">No existe la categoría.</div>'; return; }
?>
<div class="d-flex justify-content-between align-items-center mb-3">
  <h1 class="h4 m-0">Editar Categoría</h1>
  <div>
...
<form method="post" action="<?= $base ?>/doc-categorias/update/<?= (int)$row['id'] ?>" autocomplete="off" class="card p-3 shadow-sm">
  <div class="row g-3">
    <div class="col-md-4">
      <label class="form-label">Módulo</label>
      <select name="modulo" class="form-select" required>
        <?php foreach($modulos as $m): ?>
          <option value="<?= htmlspecialchars($m) ?>" <?= ($row['modulo']===$m?'selected':'') ?>><?= htmlspecialchars($m) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
    <div class="col-md-5">
    ...
    <div class="col-md-3">
      ...
      <div class="form-check">
        <input class="form-check-input" type="checkbox" name="activo" id="activo" <?= !empty($row['activo'])?'checked':'' ?>>
        <label class="form-check-label" for="activo"> Activa</label>
      </div>
    </div>
    <div class="col-12">
      <label class="form-label">Descripción</label>
      <textarea name="descripcion" class="form-control" rows="3" maxlength="500"><?= htmlspecialchars((string)($row['descripcion'] ?? '')) ?></textarea>
    </div>
  </div>
