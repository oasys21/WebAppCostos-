...
$pageTitle = $pageTitle ?? 'Proveedores';
$rows = $rows ?? [];
...
<div class="container py-3">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div><a class="btn btn-primary" href="<?= u($__base,'index.php?r=proveedores/create') ?>">Nuevo</a></div>
  </div>

  <form class="row g-2 align-items-end mb-3" method="get" action="<?= u($__base,'index.php') ?>">
    <input type="hidden" name="r" value="proveedores/index">
    ...
    <table class="table table-sm table-hover align-middle">
      ...
      <?php else: foreach($rows as $r): ?>
        <tr>
          ...
          <td class="text-end">
            <a class="btn btn-sm btn-outline-warning me-1" href="<?= u($__base,'index.php?r=proveedores/edit/'.(int)$r['id']) ?>">Editar</a>
            <form method="post" action="<?= u($__base,'index.php?r=proveedores/destroy/'.(int)$r['id']) ?>" class="d-inline" onsubmit="return confirm('¿Eliminar proveedor?');">
